...
module.exports = isCheckMode => {
  ...
  if(fileLines[1474]?.trim() === 'filter(predicate: (value: T, index: number, array: T[]) => unknown, thisArg?: any): T[];') {
    if(isCheckMode === true) {
      return 'update-needed';
    }

    fileLines.splice(1474, 1, `${fileLines[1474].replace('=> unknown', '=> boolean')} // modified from \`${filename}\``);
