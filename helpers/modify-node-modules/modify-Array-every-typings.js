...
module.exports = isCheckMode => {
  ...
  if(fileLines[1441]?.trim() === 'every(predicate: (value: T, index: number, array: T[]) => unknown, thisArg?: any): boolean;') {
    if(isCheckMode === true) {
      return 'update-needed';
    }

    fileLines.splice(1441, 1, `${fileLines[1441].replace('=> unknown', '=> boolean')} // modified from \`${filename}\``);
