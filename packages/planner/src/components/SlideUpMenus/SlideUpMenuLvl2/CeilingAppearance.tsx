...
import { closeSlideUpMenuLvl2, setCeilingParams, showSlideUpMenuLvl1, SlideUpMenuLvl2Store, useSlideUpMenuLvl2, useSpaces } from '../../../zustand';
...
export type CeilingAppearanceProps = {
  slideUpMenuLvl2: Extract<SlideUpMenuLvl2Store['slideUpMenuLvl2'], { type: 'ceilingAppearance' }>;
};

export const CeilingAppearance: React.FC<CeilingAppearanceProps> = ({ slideUpMenuLvl2: { type, isOpened, spaceId, textureType, applyToAll } }) => {
  ...
  assert(!isNull(appearanceColor) && appearanceColor.type === type, 'Something went wrong. |8rln2j|');

  const targetSpace = isNull(spaceId) ? null : spaces.find(e => e.id === spaceId);
  const targetTexture = isNullish(targetSpace) ? null : targetSpace.ceilingData[`${textureType}Texture`];
  const targetCompositeOperation = isNullish(targetSpace) ? compositeOperations[0] : targetSpace.ceilingData[`${textureType}CompositeOperation`];

  const textureAssetCategories = (
    extractTextureAssetCategories(textureAssets)
      .toSorted((a, b) => a[1].order - b[1].order)
      .toSorted((a, b) => +b[1].appearanceTypes.includes('ceilings') - +a[1].appearanceTypes.includes('ceilings'))
      .toSorted((a, b) => +isNull(a[0]) - +isNull(b[0]))
      .filter(([, { appearanceTypes, excludeFromAllMaterials }]) => ({
        all: excludeFromAllMaterials === false,
        'target-category': appearanceTypes.includes('ceilings'),
  ...
  const getCurrentTransform = () => {
    ...
    return targetSpace.ceilingData?.[key] ?? DEFAULT_TEXTURE_TRANSFORM;
  };
  ...
  useEffect(() => {
    ...
    useAppearanceColor.setState({
      appearanceColor: {
        type,
        color: targetSpace.ceilingData[`${textureType}ColorOverlay`],
      },
    });
  ...
  const colorOverlayOptions = [
    ...overlayColors.map((name, i) => ({
      id: i,
      image: getNotUndefined(textureMixedWithColors[name], 'Something went wrong. |nfx0jd|'),
      name,
    })),
    {
      ...
      name: (
        overlayColorChosenOption !== overlayColors.length
          ...
          : getHexOrHexa(getNotNull(appearanceColor.color, 'Something went wrong. |m9e9sg|').value)
      ),
    },
    ...
    >
          ...
          appearanceMenusTopBlockType === 'textures' && (
            <Animations.collapseBlock>
              <TexturesTabs
                ...
                onPick={textureId => {
                  setSearchText('');
                  assert(!isNull(spaceId), 'This should never happen. |u4ee6m|');
                  const texture = getNotUndefined(textureAssets.find(e => e.id === textureId), 'This should never happen. |wy4u80|');
                  const saved = recentTextureItems.find(e => e.textureId === textureId);
                  if(!isUndefined(saved)) {
                    setCeilingParams(spaceId, applyToAll, unionObject(`${textureType}Texture`, texture));
                    setCeilingParams(spaceId, applyToAll, unionObject(`${textureType}TextureTransform`, {
                      wScale: saved.wScale,
                      lScale: saved.lScale,
                ...
                onPickMyTexture={textureId => {
                  setSearchText('');
                  assert(!isNull(spaceId), 'This should never happen. |t0eki1|');

                  const input = getNotUndefined(myTexturesTransformed.find(e => e.id === textureId), 'This should never happen. |egz80a|');
                  const parsed = TextureAsset.parse(input);

                  setCeilingParams(spaceId, applyToAll, unionObject(`${textureType}Texture`, parsed));
                  setCeilingParams(spaceId, applyToAll, unionObject(`${textureType}TextureTransform`, DEFAULT_TEXTURE_TRANSFORM));
            ...
            <Animations.collapseBlock>
              <TextureLoader
                onPickMyTexture={parsed => {
                  setSearchText('');
                  assert(!isNull(spaceId), 'This should never happen. |lx1oya|');
      ...
      <AppearanceContainer>
        <MenuItem paddingHorizontal minHeight='unset'>
          ...
          <MenuItem paddingLeft minHeight='unset'>
            <AppearanceTabs>
              <AppearanceTab
                label={lang.slideUpMenus.appearance.labels.ceilings}
                badgeLabel={spaces.filter(e => e.ceilingData.isVisible).length.toString()}
                    ...
                    startSampler('texture', {
                      appearance(appearance) {
                        if(!isNull(appearance)) {
                          ...
                          setCeilingParams(spaceId, applyToAll, unionObject(`${textureType}TextureTransform`, isNull(transform) ? DEFAULT_TEXTURE_TRANSFORM : transform));
        ...
        searchedForTextureAssetCategories.length > 0 && (
          <MenuItem>
            <MaterialCategoryPicker
                  ...
                  image: (
                    chosenCategoryId !== id || isNull(appearanceColor.color)
                      ...
                      : getNotUndefined(
                        ...
                        'This should never happen. |140wia|',
                      )
                  ),
                }))
              }
              onClick={e => {
                setSearchText('');
                assert(!isNull(spaceId), 'This should never happen. |2t9vnn|');
                ...
                assert(!isUndefined(newTextureAsset), 'This should never happen. |w672i9|');

                setCeilingParams(spaceId, applyToAll, unionObject(`${textureType}Texture`, newTextureAsset));
                setCeilingParams(spaceId, applyToAll, unionObject(`${textureType}TextureTransform`, DEFAULT_TEXTURE_TRANSFORM));
              }}
            />
          </MenuItem>
                ...
                searchedForTextureAssets.length > 0 && (
                  <MenuItem>
                    <MaterialCategoryPicker
                      ...
                      onClick={textureId => {
                        setSearchText('');
                        assert(!isNull(spaceId), 'This should never happen. |879jow|');

                        const texture = getNotUndefined(textureAssets.find(e => e.id === textureId), 'This should never happen. |3r6cgw|');
            ...
            <MaterialCategoryPicker
              ...
              onClick={id => {
                ...
                setAppearanceColorWithUpdate(true, type, {
                  type: 'predefined',
                  value: new Color(getNotUndefined(overlayColors[id], 'This should never happen. |e408w0|')),
                });
      ...
      <MenuItem>
        <SelectRow
          ...
          onChange={e => {
            assert(!isNull(spaceId), 'This should never happen. |cm8lws|');

            setCeilingParams(spaceId, applyToAll, unionObject(`${textureType}CompositeOperation`, e));
          }}
        />
      </MenuItem>
      <AnimatePresence>
            ...
            <Animations.collapseBlock>
              ...
              <MenuItem paddingHorizontal>
                <TextField
                  ...
                  size='lg'
                  value={isNullish(targetSpace) ? '' : targetSpace.ceilingData[`comment${capitalize(textureType)}TextureOverlayColor`]}
                  onChange={e => {
                    assert(!isNull(spaceId), 'This should never happen. |l54obv|');

                    setCeilingParams(spaceId, applyToAll, unionObject(`comment${capitalize(textureType)}TextureOverlayColor`, e));
                  }}
                />
              </MenuItem>
            </Animations.collapseBlock>
                  ...
                  <Animations.collapseBlock>
                    <CompareWithOriginal
                      ...
                      onTransformChange={t => {
                        ...
                        setCeilingParams(spaceId, applyToAll, unionObject(`${textureType}TextureTransform`, t));
                      }}
                    />
                  </Animations.collapseBlock>
                )
      ...
      <MenuItem paddingHorizontal>
        <TextField
          ...
          label={lang.slideUpMenus.appearance.name}
          size='lg'
          value={isNullish(targetSpace) ? '' : targetSpace.ceilingData[`comment${capitalize(textureType)}TextureAppearance`]}
          onChange={e => {
            assert(!isNull(spaceId), 'This should never happen. |nn0lun|');

            setCeilingParams(spaceId, applyToAll, unionObject(`comment${capitalize(textureType)}TextureAppearance`, e));
          }}
        />
      </MenuItem>
      <AppearanceContainer>
        <MenuItem paddingLeft='row 3/4' paddingRight spaceBetween minHeight='unset'>
          <Checkbox
            ...
            onClick={() => {
              assert(!isNull(spaceId), 'This should never happen. |2y2a26|');
          ...
          <Select
            ...
            options={
              ObjKeys({
                sameLevelCeilings: true,
                ceilings: true,
              } satisfies Record<typeof applyToAll.type, true>)
                .map(e => ({
                  label: lang.slideUpMenus.ceilingSettings.applyToAll(e),
                  value: e,
                }))
            }
            onChange={e => {
              assert(!isNull(spaceId), 'This should never happen. |pjc63h|');
