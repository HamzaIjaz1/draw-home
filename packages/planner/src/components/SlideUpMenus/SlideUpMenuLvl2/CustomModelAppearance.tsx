...
import { closeSlideUpMenuLvl2, setCustomModelOverrideMaterialParams, showSlideUpMenuLvl1, SlideUpMenuLvl2Store, useCustomModels, useSlideUpMenuLvl2 } from '../../../zustand';
...
export type CustomModelAppearanceProps = {
  slideUpMenuLvl2: Extract<SlideUpMenuLvl2Store['slideUpMenuLvl2'], { type: 'customModelAppearance' }>;
};

export const CustomModelAppearance: React.FC<CustomModelAppearanceProps> = ({
  slideUpMenuLvl2: {
    type,
    isOpened,
    customModelId,
  ...
  assert(!isNull(appearanceColor) && appearanceColor.type === type, 'Something went wrong. |2ksd15|');
  ...
  assert(isNullish(targetCustomModel) || targetCustomModel?.type === 'regular' || targetCustomModel?.type === 'column', 'Something went wrong. |762m4e|');

  const targetCustomModelInCatalog = isNullish(targetCustomModel) ? null : catalogData.models.find(e => e.url === targetCustomModel.url);
  const targetOverrideMaterial = (
    isNullish(targetCustomModel) || isNull(targetCustomModel.overrideMaterials)
      ? null
      : targetCustomModel.overrideMaterials[overrideMaterialName]
  );
  ...
  const textureAssetCategories = (
    extractTextureAssetCategories(textureAssets)
      ...
      .toSorted((a, b) => (
        isNullish(targetCustomModelInCatalog)
          ? 0
          // eslint-disable-next-line max-len
          : +b[1].modelCategoryIds.includes(targetCustomModelInCatalog.category.id) - +a[1].modelCategoryIds.includes(targetCustomModelInCatalog.category.id)
      ))
      ...
      .filter(([, { modelCategoryIds, excludeFromAllMaterials }]) => ({
        ...
        'target-category': (
          isNullish(targetCustomModelInCatalog)
            ? false
            : modelCategoryIds.includes(targetCustomModelInCatalog.category.id)
        ),
  ...
  const colorOverlayOptions = [
    ...overlayColors.map((name, i) => ({
      id: i,
      image: getNotUndefined(textureMixedWithColors[name], 'Something went wrong. |ko65bc|'),
      name,
    })),
    {
      ...
      name: (
        overlayColorChosenOption !== overlayColors.length
          ...
          : getHexOrHexa(getNotNull(appearanceColor.color, 'Something went wrong. |1mu21d|').value)
      ),
    },
          ...
          appearanceMenusTopBlockType === 'textures' && (
            <Animations.collapseBlock>
              <TexturesTabs
                ...
                activeTextureId={
                  isNullish(targetOverrideMaterial) ? undefined : targetOverrideMaterial.texture.id
                }
                labels={{
                ...
                onPick={textureId => {
                  setSearchText('');
                  assert(!isNull(customModelId), 'This should never happen. |v9qt50|');
                  const texture = getNotUndefined(textureAssets.find(e => e.id === textureId), 'This should never happen. |2x1uf2|');
                  const saved = recentTextureItems.find(e => e.textureId === textureId);
                  if(!isUndefined(saved)) {
                    setCustomModelOverrideMaterialParams(
                      customModelId,
                      overrideMaterialName,
                      applyToAll,
                      {
                ...
                onPickMyTexture={textureId => {
                  setSearchText('');
                  assert(!isNull(customModelId), 'This should never happen. |2kao23|');

                  const input = getNotUndefined(myTexturesTransformed.find(e => e.id === textureId), 'This should never happen. |tzx8ha|');
            ...
            <Animations.collapseBlock>
              <TextureLoader
                onPickMyTexture={parsed => {
                  setSearchText('');
                  assert(!isNull(customModelId), 'This should never happen. |z30e7n|');
      ...
      <AppearanceContainer>
        <MenuItem paddingHorizontal minHeight='unset'>
          ...
          <MenuItem paddingHorizontal minHeight='unset'>
            <AppearanceTabs>
              <AppearanceTab
                label={isNullish(targetCustomModelInCatalog) ? '' : targetCustomModelInCatalog.category.name}
                badgeLabel={(isNullish(targetCustomModel) ? 0 : customModels.filter(e => e.url === targetCustomModel.url).length).toString()}
        ...
        searchedForTextureAssetCategories.length > 0 && (
          <MenuItem>
            <MaterialCategoryPicker
                  ...
                  image: (
                    chosenCategoryId !== id || isNull(appearanceColor.color)
                      ...
                      : getNotUndefined(
                        ...
                        'This should never happen. |rt6l1b|',
                      )
                  ),
                }))
              }
              onClick={async e => {
                setSearchText('');
                assert(!isNull(customModelId), 'This should never happen. |fpz5f4|');
                ...
                assert(!isUndefined(newTextureAsset), 'This should never happen. |j6ti6k|');
                  ...
                  <MenuItem>
                    <MaterialCategoryPicker
                      ...
                      onClick={async textureId => {
                        setSearchText('');
                        assert(!isNull(customModelId), 'This should never happen. |146zva|');

                        const texture = getNotUndefined(textureAssets.find(e => e.id === textureId), 'This should never happen. |039ckx|');
                    ...
                    <MaterialCategoryPicker
                      ...
                      onClick={id => {
                        ...
                        setAppearanceColorWithUpdate(true, type, {
                          type: 'predefined',
                          value: new Color(getNotUndefined(overlayColors[id], 'This should never happen. |0qu44a|')),
                        });
              ...
              <MenuItem>
                <SelectRow
                  ...
                  onChange={compositeOperation => {
                    assert(!isNull(customModelId), 'This should never happen. |tkb4xz|');

                    setCustomModelOverrideMaterialParams(customModelId, overrideMaterialName, applyToAll, { compositeOperation });
                  }}
                />
              </MenuItem>
            </Animations.collapseBlock>
              ...
              <MenuItem paddingHorizontal>
                <TextField
                  ...
                  onChange={commentTextureOverlayColor => {
                    assert(!isNull(customModelId), 'This should never happen. |87fo33|');

                    setCustomModelOverrideMaterialParams(customModelId, overrideMaterialName, applyToAll, { commentTextureOverlayColor });
                  }}
                />
              </MenuItem>
            </Animations.collapseBlock>
      ...
      <MenuItem paddingHorizontal>
        <TextField
          ...
          onChange={commentTextureAppearance => {
            assert(!isNull(customModelId), 'This should never happen. |u20uy6|');

            setCustomModelOverrideMaterialParams(customModelId, overrideMaterialName, applyToAll, { commentTextureAppearance });
          }}
        />
      </MenuItem>
      <AnimatePresence>
              ...
              <AppearanceContainer>
                <MenuItem paddingLeft='row 3/4' paddingRight spaceBetween minHeight='unset'>
                  <Checkbox
                    ...
                    onClick={() => {
                      assert(!isNull(customModelId), 'This should never happen. |wcf1n4|');
                  ...
                  <Select
                    ...
                    options={
                      ObjKeys({
                        sameLevelSameCustomModels: true,
                        sameCustomModels: true,
                      } satisfies Record<typeof applyToAll.type, true>)
                        .map(e => ({
                          label: lang.slideUpMenus.customModelSettings.applyToAll(e),
                          value: e,
                        }))
                    }
                    onChange={e => {
                      assert(!isNull(customModelId), 'This should never happen. |4367p5|');
