...
import { closeSlideUpMenuLvl2, setWallFurnitureOverrideMaterialParams, showSlideUpMenuLvl1, SlideUpMenuLvl2Store, useSlideUpMenuLvl2, useWalls } from '../../../zustand';
...
import { replaceModelMaterial } from '../../../zustand/useModelMaterials';
...
export type WallFurnitureAppearanceProps = {
  slideUpMenuLvl2: Extract<SlideUpMenuLvl2Store['slideUpMenuLvl2'], { type: 'wallFurnitureAppearance' }>;
};

export const WallFurnitureAppearance: React.FC<WallFurnitureAppearanceProps> = ({
  slideUpMenuLvl2: {
    type,
    isOpened,
    wallFurnitureId,
  ...
  assert(!isNull(appearanceColor) && appearanceColor.type === type, 'Something went wrong. |ow11nw|');

  const allWallFurnitures = walls.filter(e => e.isVisible).flatMap(e => e.furnitures);
  const targetWallFurniture = isNull(wallFurnitureId) ? null : allWallFurnitures.find(e => e.id === wallFurnitureId);
  const targetWallFurnitureInCatalog = isNullish(targetWallFurniture) ? null : catalogData.models.find(e => e.url === targetWallFurniture.url);
  const targetOverrideMaterial = (
    isNullish(targetWallFurniture) || isNull(targetWallFurniture.overrideMaterials)
      ? null
      : targetWallFurniture.overrideMaterials[overrideMaterialName]
  );
  const targetCompositeOperation = isNullish(targetOverrideMaterial) ? compositeOperations[0] : targetOverrideMaterial.compositeOperation;
  const textureAssetCategories = (
    extractTextureAssetCategories(textureAssets)
      .toSorted((a, b) => a[1].order - b[1].order)
      .toSorted((a, b) => (
        isNullish(targetWallFurnitureInCatalog)
          ? 0
          // eslint-disable-next-line max-len
          : +b[1].modelCategoryIds.includes(targetWallFurnitureInCatalog.category.id) - +a[1].modelCategoryIds.includes(targetWallFurnitureInCatalog.category.id)
      ))
      .toSorted((a, b) => +isNull(a[0]) - +isNull(b[0]))
      .filter(([, { modelCategoryIds, excludeFromAllMaterials }]) => ({
        all: excludeFromAllMaterials === false,
        'target-category': (
          isNullish(targetWallFurnitureInCatalog)
            ? false
            : modelCategoryIds.includes(targetWallFurnitureInCatalog.category.id)
        ),
  ...
  const getCurrentTransform = () => {
    ...
    return targetOverrideMaterial?.textureTransform ?? DEFAULT_TEXTURE_TRANSFORM;
  };

  const getCurrentTextureId = () => isNullish(targetOverrideMaterial) ? null : targetOverrideMaterial.texture.id;
  ...
  const [chosenCategoryId, setChosenCategoryId] = useState(() => {
    const category = (
      isNullish(targetOverrideMaterial)
        ? null
        : textureAssetCategories.find(e => e[1].textureAssetIds.includes(targetOverrideMaterial.texture.id))
    );
  ...
  useEffect(() => {
    if(isNullish(targetOverrideMaterial)) {
      return;
    }
    ...
    useAppearanceColor.setState({
      appearanceColor: {
        type,
        color: targetOverrideMaterial.colorOverlay,
      },
    });
  }, [targetOverrideMaterial, type]);
  ...
  const textureMixedWithColors = useTextureMixedWithColorsCached(
    getImageWithDefault(targetOverrideMaterial?.texture.attributes.maps.colorMap.data.attributes.formats?.thumbnail.url),
    overlayColors,
  ...
  const colorOverlayOptions = [
    ...overlayColors.map((name, i) => ({
      id: i,
      image: getNotUndefined(textureMixedWithColors[name], 'Something went wrong. |fia35j|'),
      name,
    })),
    {
      ...
      name: (
        overlayColorChosenOption !== overlayColors.length
          ...
          : getHexOrHexa(getNotNull(appearanceColor.color, 'Something went wrong. |y6zj50|').value)
      ),
    },
  ...
  const previewImage = preferImage([
    targetOverrideMaterial?.texture.attributes.maps.colorMap.data.attributes.formats?.thumbnail.url,
    targetOverrideMaterial?.texture.attributes.maps.colorMap.data.attributes.url,
  ]);
    ...
    >
          ...
          appearanceMenusTopBlockType === 'textures' && (
            <Animations.collapseBlock>
              <TexturesTabs
                ...
                activeTextureId={isNullish(targetOverrideMaterial) ? undefined : targetOverrideMaterial.texture.id}
                labels={{
                ...
                onPick={textureId => {
                  setSearchText('');
                  assert(!isNull(wallFurnitureId), 'This should never happen. |3xcr0e|');
                  const texture = getNotUndefined(textureAssets.find(e => e.id === textureId), 'This should never happen. |v2a2b2|');
                  if(texturesTab === 'recent') {
                    const saved = recentTextureItems.find(e => e.textureId === textureId);
                    if(!isUndefined(saved)) {
                      setWallFurnitureOverrideMaterialParams(
                        wallFurnitureId,
                        overrideMaterialName,
                        applyToAll,
                        {
                ...
                onPickMyTexture={textureId => {
                  setSearchText('');
                  assert(!isNull(wallFurnitureId), 'This should never happen. |vqj64p|');

                  const input = getNotUndefined(myTexturesTransformed.find(e => e.id === textureId), 'This should never happen. |1qa107|');
            ...
            <Animations.collapseBlock>
              <TextureLoader
                onPickMyTexture={parsed => {
                  setSearchText('');
                  assert(!isNull(wallFurnitureId), 'This should never happen. |skz03f|');
      ...
      <AppearanceContainer>
        <MenuItem paddingHorizontal minHeight='unset'>
          ...
          <MenuItem paddingHorizontal minHeight='unset'>
            <AppearanceTabs>
              <AppearanceTab
                label={isNullish(targetWallFurnitureInCatalog) ? '' : targetWallFurnitureInCatalog.category.name}
                badgeLabel={
                  (isNullish(targetWallFurniture) ? 0 : allWallFurnitures.filter(e => e.type === targetWallFurniture.type).length).toString()
                }
        ...
        searchedForTextureAssetCategories.length > 0 && (
          <MenuItem>
            <MaterialCategoryPicker
                  ...
                  image: (
                    chosenCategoryId !== id || isNull(appearanceColor.color)
                      ...
                      : getNotUndefined(
                        ...
                        'This should never happen. |15x87n|',
                      )
                  ),
                }))
              }
              onClick={async e => {
                setSearchText('');
                assert(!isNull(wallFurnitureId), 'This should never happen. |90c77f|');
                ...
                assert(!isUndefined(newTextureAsset), 'This should never happen. |nv9mbi|');
                ...
                setWallFurnitureOverrideMaterialParams(
                  ...
                  applyToAll,
                  {
                    texture: newTextureAsset,
                    textureTransform: DEFAULT_TEXTURE_TRANSFORM,
                  },
                );
                await replaceModelMaterial(overrideMaterialName, newTextureAsset);
              }}
            />
          </MenuItem>
        )
            ...
            <Animations.collapseBlock>
              ...
              </AppearanceContainer>
              {
                searchedForTextureAssets.length > 0 && (
                  <MenuItem>
                    <MaterialCategoryPicker
                      chosenOption={isNullish(targetOverrideMaterial) ? undefined : targetOverrideMaterial.texture.id}
                      options={
                        searchedForTextureAssets.map(({ id, attributes: { name, preview } }) => ({
                          ...
                          image: getImageWithDefault(
                            preferImage([
                              isNull(appearanceColor.color) || isNullish(targetOverrideMaterial) || id !== targetOverrideMaterial.texture.id
                                ? undefined
                                : textureMixedWithColors[getHexOrHexa(appearanceColor.color.value)],
                      ...
                      onClick={async textureId => {
                        setSearchText('');
                        assert(!isNull(wallFurnitureId), 'This should never happen. |3y6600|');

                        const texture = getNotUndefined(textureAssets.find(e => e.id === textureId), 'This should never happen. |1h9mg3|');
                        ...
                        await replaceModelMaterial(overrideMaterialName, texture);
                      }}
                    />
                  </MenuItem>
                )
                    ...
                    <MaterialCategoryPicker
                      ...
                      onClick={id => {
                        ...
                        setAppearanceColorWithUpdate(true, type, {
                          type: 'predefined',
                          value: new Color(getNotUndefined(overlayColors[id], 'This should never happen. |iz0lwi|')),
                        });
              ...
              <MenuItem>
                <SelectRow
                  ...
                  onChange={compositeOperation => {
                    assert(!isNull(wallFurnitureId), 'This should never happen. |vkd3gi|');

                    setWallFurnitureOverrideMaterialParams(wallFurnitureId, overrideMaterialName, applyToAll, { compositeOperation });
                  }}
                />
              </MenuItem>
            </Animations.collapseBlock>
            ...
            <Animations.collapseBlock>
              ...
              <MenuItem paddingHorizontal>
                <TextField
                  ...
                  size='lg'
                  value={isNullish(targetOverrideMaterial) ? '' : targetOverrideMaterial.commentTextureOverlayColor}
                  onChange={commentTextureOverlayColor => {
                    assert(!isNull(wallFurnitureId), 'This should never happen. |ksg73n|');

                    setWallFurnitureOverrideMaterialParams(wallFurnitureId, overrideMaterialName, applyToAll, { commentTextureOverlayColor });
                  }}
                />
              </MenuItem>
            </Animations.collapseBlock>
      ...
      <MenuItem paddingHorizontal>
        <TextField
          ...
          label={lang.slideUpMenus.appearance.name}
          size='lg'
          value={isNullish(targetOverrideMaterial) ? '' : targetOverrideMaterial.commentTextureAppearance}
          onChange={commentTextureAppearance => {
            assert(!isNull(wallFurnitureId), 'This should never happen. |9j7iex|');

            setWallFurnitureOverrideMaterialParams(wallFurnitureId, overrideMaterialName, applyToAll, { commentTextureAppearance });
          }}
        />
      </MenuItem>
      <AnimatePresence>
              ...
              <AppearanceContainer>
                <MenuItem paddingLeft='row 3/4' paddingRight spaceBetween minHeight='unset'>
                  <Checkbox
                    ...
                    onClick={() => {
                      assert(!isNull(wallFurnitureId), 'This should never happen. |um765s|');
                  ...
                  <Select
                    ...
                    options={
                      ObjKeys({
                        sameLevelSameWallFurnitures: true,
                        sameWallFurnitures: true,
                      } satisfies Record<typeof applyToAll.type, true>)
                        .map(e => ({
                          label: lang.slideUpMenus.wallFurnitureSettings.applyToAll(e),
                          value: e,
                        }))
                    }
                    onChange={e => {
                      assert(!isNull(wallFurnitureId), 'This should never happen. |ng0t09|');
