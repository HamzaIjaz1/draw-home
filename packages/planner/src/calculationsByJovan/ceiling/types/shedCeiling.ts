...
export class ShedCeiling extends BaseCeiling {
  ...
  private buildGeometry(): void {
    const { coords, depth = 5, slopeAngle = 15 } = this.params;

    if(!coords || coords.length < 3) {
      throw new Error('At least 3 coords are required for a shed ceiling');
    }
