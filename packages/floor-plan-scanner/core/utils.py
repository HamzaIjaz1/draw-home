...
def drawSegmentationImage(segmentations, numColors=42, blackIndex=-1, blackThreshold=-1):
    try:
        # Input validation
        if segmentations is None or segmentations.size == 0:
            print("Warning: Empty segmentation data")
            return np.zeros((256, 256, 3), dtype=np.uint8)

        # Ensure we have valid dimensions
        if segmentations.ndim not in [2, 3]:
            print("Warning: Segmentation data has invalid dimensions:", segmentations.ndim)
            return np.zeros((segmentations.shape[0], segmentations.shape[1], 3), dtype=np.uint8)

        if segmentations.ndim == 2:
            numColors = max(numColors, int(segmentations.max()) + 2)
        else:
            if blackThreshold > 0:
        ...
        colorPalette = ColorPalette(numColors)
        randomColor = colorPalette.getColorMap()
        if blackIndex >= 0:
        ...
        if segmentations.ndim == 3:
            # Get argmax for each pixel position
        ...
        # Create the output image by mapping indices to colors
        # Clip the segmentation to valid indices to avoid out of bounds access
        segmentation = np.clip(segmentation, 0, numColors-1)

        # Create the visualization image
        result = randomColor[segmentation.reshape(-1)].reshape((height, width, 3)).astype(np.uint8)
        return result
    except Exception as e:
        print(f"Error in drawSegmentationImage: {str(e)}")
...
def drawWallMask(walls, width, height, thickness=3, indexed=False):
    if indexed:
        ...
        for wall in walls:
            ...
            continue
        wallMask = wallMask.astype(np.bool_)
        pass
    return wallMask
